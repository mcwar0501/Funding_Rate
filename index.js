...
cron.schedule("18 * * * *", async () => {
    ...
    const negativeValues = fetchedData.filter(item => item.value <= -0.1);
...
(async () => {
    await fetchAllFundingRates();
    const negativeValues = fetchedData.filter(item => item.value <= -0.1);  // Lọc dữ liệu
